...
// Admin can only create Admin or Discipline Incharge (students register via student registration)
const ROLES = [
...
export default function CreateUserModal({ onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await apiPost("/api/users", { name: name.trim() || undefined, email: email.trim(), password, role });
      onSaved?.();
      onClose();
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {saving ? <><Loader2 className="animate-spin" size={18} /> Creating...</> : "Create User"}
            </button>
          </div>
        </form>
      </div>
